...
function validateBackendUrl(url) {
  try {
    ...
    // Prevent local/private IPs for security
    ...
    if (
      ...
      throw new Error('Cannot proxy to local/private IP addresses');
    }
...
function validateApiPrefix(prefix) {
  ...
  if (cleanPrefix === '/') {
    throw new Error('Cannot proxy all requests (use specific path like /api)');
  }
  
  if (!/^\/[a-z0-9-\/]+$/i.test(cleanPrefix)) {
    throw new Error('API prefix contains invalid characters');
  }
 ...
 * Now supports backend proxy integration
 */
async function createVirtualHost(subdomain, hasBackend = false, backendUrl = null, apiPrefix = null) {
  ...
  // Base configuration
  let vhostConfig = `<VirtualHost *:443>
...
`;
  ...
  // Add backend proxy configuration if enabled
  if (hasBackend && backendUrl && apiPrefix) {
    vhostConfig += `    # Backend API Proxy Configuration
    ProxyPreserveHost On
    ProxyPass ${apiPrefix} ${backendUrl}${apiPrefix}
    ProxyPassReverse ${apiPrefix} ${backendUrl}${apiPrefix}
...
`;
  ...
  vhostConfig += `    <Directory ${documentRoot}>
  ...
  // Add rewrite condition to exclude API prefix if backend is enabled
  if (hasBackend && apiPrefix) {
    vhostConfig += `
            RewriteCond %{REQUEST_URI} !^${apiPrefix.replace(/\//g, '\\/')}`;
  }
...
app.post('/deploy', authenticateRequest, async (req, res) => {
  const { projectId, subdomain, files, hasBackend, backendUrl, apiPrefix } = req.body;
  ...
  try {
    ...
    console.log(`Backend integration: ${hasBackend ? 'enabled' : 'disabled'}`);
    
    // Validate subdomain
    const cleanSubdomain = validateSubdomain(subdomain);
    const projectPath = path.join(BASE_DIR, cleanSubdomain);
    
    // Validate backend configuration if enabled
    let validatedBackendUrl = null;
    let validatedApiPrefix = null;
    
    if (hasBackend) {
      if (!backendUrl || !apiPrefix) {
        throw new Error('Backend URL and API prefix are required when backend integration is enabled');
      }
      
      validatedBackendUrl = validateBackendUrl(backendUrl);
      validatedApiPrefix = validateApiPrefix(apiPrefix);
      
      console.log(`Backend URL: ${validatedBackendUrl}`);
      console.log(`API Prefix: ${validatedApiPrefix}`);
    }
    ...
    // Create Apache virtual host with backend proxy support
...
app.listen(PORT, () => {
  ...
  console.log(`Backend proxy support: enabled`);
});
